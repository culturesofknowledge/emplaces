...
Example (adapted from plinked-places](https://github.com/LinkedPasts/linked-places):
...
While it's possible to capoture the intended `lpo` sementics in the OWL Time ontology, the result isn't pretty, especially if represented in JSON-LD.

Hence, I'd suggest using the easier-to-use `lpo` terms in the JSON-LD context, and maintain the OWL Time mapping, maybe inthe form of a set of inference rules, so that information exchange compatiblity with OWL Time can be achieved.
